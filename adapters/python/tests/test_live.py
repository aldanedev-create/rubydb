...
@unittest.skipUnless(
    os.getenv("RUBYDB_LIVE_URL"),
    "set RUBYDB_LIVE_URL to run against a live RubyDB server",
)
    ...
    def test_parameterized_query_transaction_and_prepared_statement(self):
        with rubydb.connect(os.environ["RUBYDB_LIVE_URL"]) as connection:
            ...
            with connection.cursor() as cursor:
                ...
                cursor.execute(
                    "INSERT INTO python_adapter_smoke (name) VALUES (?)", ["python"]
                )
                cursor.execute(
                    "SELECT name FROM python_adapter_smoke WHERE name = ?", ["python"]
                )
                self.assertEqual(cursor.fetchone()["name"], "python")

            statement = connection.prepare(
                "SELECT name FROM python_adapter_smoke WHERE name = ?"
            )
            try:
